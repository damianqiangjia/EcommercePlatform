  ...
  export default{
    ...
    methods: {
      draw(){
        ...
        this.$http.get('http://127.0.0.1:8000/api/show_areadata')
        .then((response) => {
        ...
        var worldone = echarts.init(document.getElementById('worldone'));
    ...
    series: [
        ...
        var worldtwo = echarts.init(document.getElementById('worldtwo'));
        // 绘制图表
        worldtwo.setOption({
              title : {
        text: '商品种类构成',
    ...
    this.$http.get('http://127.0.0.1:8000/api/show_worlddata')
        .then((response) => {
            var res = JSON.parse(response.bodyText)
            if (res.error_num == 0) {
              for(var i = 0; i < 6; i++){
